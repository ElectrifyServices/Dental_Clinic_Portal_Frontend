...
export function MedicalRecordsPage({ 
  ...
  const patientGroups = patients.map(patient => {
    const patientTreatments = treatments.filter(t => 
      ...
      (t.patientName && t.patientName.toLowerCase() === patient.name.toLowerCase())
    );
    const patientInvoices = invoices.filter(inv => 
      inv.patientId === patient.id || 
      (inv.patientName && inv.patientName.toLowerCase() === patient.name.toLowerCase())
    );
    const patientAppointments = appointments.filter(apt => 
      apt.patientId === patient.id || 
      (apt.patientName && apt.patientName.toLowerCase() === patient.name.toLowerCase())
    );
    const patientManualRecords = emrRecords.filter(r => 
      r.patientId === patient.id || 
      (r.patientName && r.patientName.toLowerCase() === patient.name.toLowerCase())
    );
    ...
    return {
      ...
      lastActivity: timeline[0]?.date || patient.createdAt || new Date().toISOString(),
