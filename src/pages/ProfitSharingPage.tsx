...
export const ProfitSharingPage: React.FC<ProfitSharingPageProps> = ({ treatments, doctorsWithSchedules }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      {dateFilter === 'custom' && (
        <div className="flex items-center gap-3 p-4 bg-white rounded-xl border border-gray-100 shadow-sm animate-in slide-in-from-top-2">
          <input
            type="date"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Revenue', value: stats.totalRev, icon: TrendingUp, color: 'text-blue-600', bg: 'bg-blue-50' },
      ...
      <div className="space-y-4">
        {doctorsWithSchedules.map((doctor) => {
          ...
          return (
            <div key={doctor.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden transition-all duration-300">
              <div 
              ...
              >
                ...
                <div className="flex items-center gap-8">
                  <div className="text-right">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Estimated Share</p>
                    <p className="text-lg font-bold text-green-600">₹{profitShare.toLocaleString()}</p>
                  </div>
                  <div className={`p-2 rounded-lg bg-gray-50 text-gray-400 transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`}>
              ...
              {isExpanded && (
                <div className="px-5 pb-5 border-t border-gray-50 animate-in slide-in-from-top-2">
                  {doctorTreatments.length > 0 ? (
                    <div className="overflow-x-auto mt-4">
                      <table className="w-full">
                        <thead>
                          <tr className="text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider border-b border-gray-50">
                        ...
                        <tbody className="divide-y divide-gray-50">
                          {doctorTreatments.map(t => (
                            <tr key={t.id} className="text-sm">
                              <td className="py-4 font-semibold text-gray-800">{t.patientName}</td>
                              <td className="py-4 text-gray-600">{t.procedure}</td>
                              <td className="py-4 text-gray-400 text-xs">
                                {new Date(t.date || t.createdAt).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}
                              </td>
                              <td className="py-4 font-bold text-gray-900 text-right">₹{Number(t.cost).toLocaleString()}</td>
                    ...
                    <div className="py-8 text-center bg-gray-50 rounded-xl mt-4">
                      ...
                      <p className="text-sm text-gray-400 font-medium">No procedures found for this period</p>
                    </div>
                  )}
                </div>
              )}
