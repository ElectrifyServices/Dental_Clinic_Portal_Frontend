...
<div style="display:flex; flex-direction:row; align-items:center; margin-bottom:4px;">
  ${iconLocation}<span style="font-size:10.5px; font-weight:700;">${CLINIC_ADDRESS}</span>
</div>
<div style="display:flex; flex-direction:row; align-items:center; gap:20px; margin-bottom:4px;">
  <div style="display:flex; flex-direction:row; align-items:center;">${iconPhone}<span style="font-size:10px; font-weight:500;">${CLINIC_PHONE}</span></div>
  <div style="display:flex; flex-direction:row; align-items:center;">${iconEmail}<span style="font-size:10px; font-weight:500;">${CLINIC_EMAIL}</span></div>
  <div style="display:flex; flex-direction:row; align-items:center;">${iconInstagram}<span style="font-size:10px; font-weight:500;">${CLINIC_INSTAGRAM}</span></div>
</div>
...
export const generateInvoicePDF = async (invoice: any, patient: any) => {
  ...
  const itemsHtml = items
    .map((item: any, i: number) => {
      ...
      <tr style="border-bottom:1px solid ${LINE};">
        ...
        <td style="padding:12px 12px; font-size:11px; font-weight:700; color:${INK}; text-align:right; width:15%;">${formatCurrency(billedVal)}</td>
      </tr>
    `;
    })
    .join("");
  ...
  const signatureBlock = `
    <div style="text-align:right; font-family:'Inter',sans-serif; margin-right:12px;">
      <div style="font-size:13px; font-weight:800; color:${INK};">${displayDoctorName}</div>
      <div style="font-size:11px; color:${INK_MUTED}; font-weight:600; margin-top:2px;">Authorized Signatory</div>
      <div style="font-size:10px; color:${INK_MUTED}; margin-top:2px;">Opal Smiles Dental Studio</div>
    </div>
  `;
  ...
  const htmlContent = `
    <div style="width:794px; background:#fff; margin:0; padding:0; color:${INK}; display:flex; flex-direction:column; min-height:1123px; box-sizing:border-box; font-family:'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
      ...
      <div style="text-align:center; font-size:13px; font-weight:700; letter-spacing:2px; text-transform:uppercase; padding: 12px 0 6px;">
        ${"Invoice"}
      </div>
          ...
          <table style="width:100%; border-collapse:collapse; margin-top:8px; border-bottom:1.5px solid ${LINE};">
            <thead>
              <tr style="border-top:1.5px solid ${LINE}; border-bottom:1.5px solid ${LINE}; background:#fafafa;">
                ...
                <th style="padding:10px 12px; text-align:right; font-size:10px; font-weight:800; color:${INK}; text-transform:uppercase; letter-spacing:0.4px; width:15%;">Total Amount</th>
                <th style="padding:10px 12px; text-align:right; font-size:10px; font-weight:800; color:${INK}; text-transform:uppercase; letter-spacing:0.4px; width:15%;">Billed Amount</th>
              </tr>
            </thead>
            <tbody>
              ${itemsHtml}
          ...
          <div style="min-width:280px; display:flex; flex-direction:column; gap:4px;">
            <table style="width:100%; border-collapse:collapse; font-size:12px; color:${INK};">
              <tbody>
                ...
                <tr>
                  <td style="padding:5px 0; color:${INK_MUTED}; font-weight:600;">Billed Amount</td>
                  <td style="padding:5px 0; text-align:right; font-weight:700;">${formatCurrency(items.reduce((s: number, it: any) => s + Number(it.billed_amount || it.amount || 0), 0))}</td>
                </tr>
              </tbody>
            </table>
