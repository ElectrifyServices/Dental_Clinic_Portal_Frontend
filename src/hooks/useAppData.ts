...
export const useAppData = () => {
  ...
  const { patients, setPatients } = patientData;
  const { invoices, setInvoices } = invoiceData;
  const { treatments, setTreatments, completedConsultations, setCompletedConsultations } = treatmentData;
  ...
  const handleSaveInvoice = (invoice: any) => {
    setInvoices(prev => {
      ...
      setPatients(pp => pp.map(p => {
        if (p.id === invoice.patientId || p.name === invoice.patientName) {
          return { ...p, outstandingBalance: (p.outstandingBalance || 0) + (invoice.total || invoice.amount || 0) };
        }
        return p;
      }));
      // Mark linked consultations & treatments as billed
      if (invoice.linkedItemIds?.length) {
        setCompletedConsultations(cc => cc.map(c =>
          invoice.linkedItemIds.includes(c.id) ? { ...c, isBilled: true } : c
        ));
        setTreatments(tt => tt.map(t => {
          let modified = false;
          let updated = { ...t };
          if (invoice.linkedItemIds.includes(t.id)) { updated.isBilled = true; modified = true; }
          if (Array.isArray(t.sessions)) {
            const sessions = t.sessions.map((s: any) => {
              if (invoice.linkedItemIds.includes(`${t.id}-${s.id}`)) { modified = true; return { ...s, isBilled: true }; }
              return s;
            });
      ...
      if (inv && inv.status !== 'paid') {
        setPatients(pp => pp.map(p => {
          if (p.id === inv.patientId || p.name === inv.patientName) {
            return { ...p, outstandingBalance: Math.max(0, (p.outstandingBalance || 0) - (inv.total || inv.amount || 0)) };
          }
          return p;
        }));
      }
  ...
  const handleCompleteConsultation = (consultation: any) => {
    ...
    if (consultation.treatmentPlans?.length) {
      const plans = consultation.treatmentPlans.map((plan: any) => ({
        ...
        patientId: consultation.patientId, patientName: consultation.patientName,
        procedure: plan.procedure, tooth: plan.tooth,
        ...
        cost: plan.cost || 0, status: plan.isActive ? 'in-progress' : 'planned',
        doctorId: consultation.doctorId || '1', doctorName: consultation.doctorName || 'Dr. Sharma',
        sessions: plan.sessions || [], prescriptions: consultation.prescriptions || [],
      }));
      setTreatments(prev => {
  ...
  const handleDeleteEmployee = (id: string) => {
    ...
    setPatients(prev => prev.map(p =>
      p.corporateMemberId === id
        ? { ...p, corporatePlanId: '', corporatePlanName: '', corporateMemberId: '' }
        : p
    ));
  };
