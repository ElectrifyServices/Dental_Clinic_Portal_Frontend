...
export function getDashboardDateRange(period: string, customStart?: string, customEnd?: string) {
  const now = new Date();
  let startDate: Date;
  let endDate: Date;
  ...
  switch (period) {
    ...
    case 'custom':
      ...
      endDate = customEnd ? new Date(customEnd) : new Date(now.setHours(23, 59, 59, 999));
      break;
    default:
      startDate = new Date(now.setHours(0, 0, 0, 0));
      endDate = new Date(now.setHours(23, 59, 59, 999));
...
export const useAppointmentsCount = (period: string, customStart?: string, customEnd?: string) => {
  const { startDate, endDate } = getDashboardDateRange(period, customStart, customEnd);
...
export const useRevenue = (period: string, customStart?: string, customEnd?: string) => {
  const { startDate, endDate } = getDashboardDateRange(period, customStart, customEnd);
  return useApiQuery<any>({
...
export const usePatientsCount = (period: string, customStart?: string, customEnd?: string) => {
  const { startDate, endDate } = getDashboardDateRange(period, customStart, customEnd);
  return useApiQuery<any>({
...
export const useCorporateMembers = () => {
  return useApiQuery<any>({
    queryKey: ['dashboard', 'corporate-members'],
...
export const useAppointmentStatusBreakdown = (period: string, customStart?: string, customEnd?: string) => {
  const { startDate, endDate } = getDashboardDateRange(period, customStart, customEnd);
  return useApiQuery<any>({
...
export const useDoctorPerformance = (page: number = 1, limit: number = 5, search?: string) => {
  return useApiQuery<any>({
...
export const useRecentPatients = (page: number = 1, limit: number = 5, search?: string) => {
  return useApiQuery<any>({
