      ...
      <div className="mt-auto border-t border-border p-3 bg-muted/20 flex-shrink-0">
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-2 mb-3 bg-card p-2 rounded-xl border border-border shadow-sm">
            <div
              className={`w-7 h-7 rounded-lg flex items-center justify-center text-white font-bold text-xs shadow-md transition-colors duration-500 ${themeClasses.split(" ")[0]}`}
            >
              {state.user?.name?.[0]}
            </div>
            <div className="min-w-0">
              <p className="text-[11px] font-bold text-foreground truncate leading-tight uppercase">
                {state.user?.name}
              </p>
