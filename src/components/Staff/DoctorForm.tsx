...
import { staffSchema, type StaffFormData, staffStep1Fields, staffStep2Fields, staffStep3Fields } from '@/lib/schemas/staff.schema';
...
export function DoctorForm({ onClose, onSave, doctor }: DoctorFormProps) {
  ...
  const onSubmit = (data: StaffFormData) => {
    onSave({
      ...
      salaryPending: !doctor ? (parseFloat(data.monthlySalary ?? '0') || 0).toLocaleString('en-IN') : data.salaryPending,
  ...
  const handleChange = (e: any) => {
    ...
    form.setValue(name as keyof StaffFormData, type === 'checkbox' ? checked : value, { shouldValidate: true });
  };

  const handlePermissionChange = (id: string, checked: boolean) => {
    const current = form.getValues('permissions');
    form.setValue('permissions', checked ? [...current, id] : current.filter(p => p !== id));
  };
  ...
  const handleDocumentRemove = (docType: string) => {
    const current = form.getValues('documents');
    form.setValue('documents', current.filter((d: any) => d.type !== docType));
  };
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 2: return <Step2Role formData={formData} onChange={(role) => form.setValue('role', role as any)} onPermissionChange={handlePermissionChange} />;
    ...
    <Modal
      ...
      footer={
        <div className="flex justify-between items-center w-full">
          <Button variant="outline" onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : onClose()} className="gap-2">
            <ChevronLeft className="w-4 h-4" /> {currentStep === 1 ? 'Cancel' : 'Previous Step'}
          </Button>
          <div className="flex gap-3">
            <Button onClick={currentStep < 4 ? handleNextStep : form.handleSubmit(onSubmit)} className="gap-2">
