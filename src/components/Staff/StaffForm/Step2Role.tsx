...
const PERMISSIONS = [
  { id: 'appointments', label: 'Appointments', description: 'Manage bookings', icon: Calendar },
  { id: 'patients', label: 'Patients', description: 'View demographics', icon: User },
  { id: 'treatments', label: 'Treatments', description: 'Create plans', icon: Stethoscope },
  { id: 'emr', label: 'EMR', description: 'Access records', icon: FileText },
  { id: 'billing', label: 'Billing', description: 'Handle payments', icon: DollarSign },
  { id: 'inventory', label: 'Inventory', description: 'Track supplies', icon: FileText },
  { id: 'reports', label: 'Reports', description: 'View analytics', icon: FileText }
];

export function Step2Role({ formData, onChange, onPermissionChange }: Step2Props) {
  return (
    <div className="space-y-8 py-4">
      <div className="text-center">
        <h3 className="text-lg font-black text-foreground uppercase tracking-tight">Access Control</h3>
        <p className="text-xs text-muted-foreground font-medium">Define organizational role and modular permissions</p>
      </div>
          ...
          return (
            <button key={role.value} type="button" onClick={() => onChange(role.value)}
              ...
              <div className="flex items-center gap-3 mb-2">
                ...
                <h4 className="font-black text-foreground uppercase text-xs tracking-wider">{role.label}</h4>
              </div>
              <p className="text-[11px] text-muted-foreground font-medium leading-tight">{role.description}</p>
              {isActive && <div className="absolute top-3 right-3 w-2 h-2 bg-primary rounded-full animate-pulse" />}
            </button>
          );
        })}
      </div>
      ...
      {formData.role !== 'admin' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-black text-foreground uppercase tracking-widest">Granular Permissions</h4>
            <Badge variant="gray" className="text-[9px]">{formData.permissions.length} Enabled</Badge>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {PERMISSIONS.map(p => (
