...
export function DoctorScheduleManager({
  ...
  const generateTimeSlots = (dayKey: string) => {
    ...
    const breakS = day.breakStart
      ? new Date(`2024-01-01 ${day.breakStart}`)
      : null;
    ...
    while (current < end) {
      if (breakS && breakE && current >= breakS && current < breakE) {
        current = new Date(breakE);
        continue;
      }
      slots.push(current.toTimeString().slice(0, 5));
      current.setMinutes(
        current.getMinutes() + settings.duration + settings.bufferTime,
      );
    }
    return slots;
  };
          ...
          <div className="space-y-3">
            {DAYS.map((day) => {
              ...
              return (
                <div
                  key={day.key}
                  className={`p-4 border rounded-2xl transition-all ${isWorking ? "border-primary/20 bg-card" : "border-border bg-muted/20 opacity-60"}`}
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-[10px] uppercase transition-colors ${isWorking ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                      >
                        {day.label.slice(0, 3)}
                      </div>
                  ...
                  {isWorking && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <LabeledField label="Break Start">
                          <input
                            type="time"
