                  ...
                  {isWorking && (
                    <div className="space-y-4 animate-in fade-in duration-300">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <LabeledField label="Start Time"><input type="time" value={schedule[day.key]?.startTime} onChange={(e) => handleDayChange(day.key, 'startTime', e.target.value)} className="w-full px-3 py-1.5 border rounded-xl text-xs font-bold" /></LabeledField>
                        <LabeledField label="End Time"><input type="time" value={schedule[day.key]?.endTime} onChange={(e) => handleDayChange(day.key, 'endTime', e.target.value)} className="w-full px-3 py-1.5 border rounded-xl text-xs font-bold" /></LabeledField>
                        <LabeledField label="Break Start"><input type="time" value={schedule[day.key]?.breakStart} onChange={(e) => handleDayChange(day.key, 'breakStart', e.target.value)} className="w-full px-3 py-1.5 border rounded-xl text-xs font-medium text-muted-foreground bg-muted/30" /></LabeledField>
                        <LabeledField label="Break End"><input type="time" value={schedule[day.key]?.breakEnd} onChange={(e) => handleDayChange(day.key, 'breakEnd', e.target.value)} className="w-full px-3 py-1.5 border rounded-xl text-xs font-medium text-muted-foreground bg-muted/30" /></LabeledField>
                      </div>
