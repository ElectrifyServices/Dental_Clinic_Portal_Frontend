    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
      <div className="bg-card rounded-[2rem] max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border border-white">
        <div className="p-8 border-b border-border flex items-center justify-between sticky top-0 bg-card z-10">
          ...
          <Button variant="ghost" onClick={onClose} className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center text-muted-foreground/60 hover:text-muted-foreground">
            <X className="w-5 h-5" />
          </Button>
        </div>
        ...
        <div className="p-8 space-y-6">
          ...
          <form onSubmit={(e) => { e.preventDefault(); onSubmit({}); }} className="space-y-6">
            ...
            <div className="flex justify-end gap-3 pt-6 border-t border-border">
               <Button variant="ghost" type="button" onClick={onClose} className="px-6 py-3 text-sm font-bold text-muted-foreground/60 hover:text-muted-foreground">Cancel</Button>
