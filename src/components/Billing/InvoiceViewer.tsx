...
export function InvoiceViewer({ invoiceId, onClose, onUpdateStatus }: InvoiceViewerProps) {
  ...
  const corporatePlan = invoice.corporatePlanId ? corporatePlans.find(cp => cp.id === invoice.corporatePlanId) : null;
  ...
  const statusColors: Record<string, 'green' | 'blue' | 'red' | 'gray' | 'violet'> = {
    paid: 'green',
    sent: 'blue',
    overdue: 'red',
    draft: 'gray',
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 w-full">
          <div className="flex gap-2">
            {invoice.status === 'draft' && onUpdateStatus && (
              <Button onClick={() => onUpdateStatus(invoice.id, 'sent')} variant="outline" className="gap-2">
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-border overflow-hidden">
          <table className="w-full text-left text-sm">
            <thead className="bg-muted/50 border-b border-border">
              <tr>
                <th className="px-4 py-3 font-bold text-muted-foreground uppercase text-[10px] tracking-wider">Description</th>
                <th className="px-4 py-3 font-bold text-muted-foreground uppercase text-[10px] tracking-wider text-center">Qty</th>
                <th className="px-4 py-3 font-bold text-muted-foreground uppercase text-[10px] tracking-wider text-right">Rate</th>
                <th className="px-4 py-3 font-bold text-muted-foreground uppercase text-[10px] tracking-wider text-right">Amount</th>
              </tr>
            </thead>
            ...
            <tfoot className="bg-muted/30 font-medium">
              ...
              <tr className="bg-primary/5 text-lg font-black">
                <td colSpan={3} className="px-4 py-4 text-right text-primary uppercase tracking-wider">Grand Total</td>
                <td className="px-4 py-4 text-right text-primary">₹{invoice.total.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
        </div>
        ...
        {invoice.isComplimentary && invoice.complimentaryNote && (
          <div className="p-4 bg-violet-50 border border-violet-100 rounded-2xl flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-bold text-violet-700 uppercase tracking-wider">Complimentary Billing Note</p>
              <p className="text-sm text-violet-800 font-medium mt-1 italic">"{invoice.complimentaryNote}"</p>
            </div>
          </div>
        )}
      </div>
