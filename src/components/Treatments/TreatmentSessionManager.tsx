      ...
      <div className="modal-box max-w-5xl w-full">
        <div className="modal-header bg-gradient-to-r from-blue-50 to-indigo-50/30">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5 font-medium">{patientName} • {procedure}</p>
          </div>
        ...
        <div className="modal-body p-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-blue-50/50 p-6 rounded-3xl border border-blue-100 shadow-sm">
              <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mb-1">Total Sessions</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div key={session.id} className={`rounded-2xl p-5 border transition-all ${getStatusColor(session.status)} shadow-sm hover:shadow-md`}>
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center font-bold text-sm shadow-sm border border-inherit">
                      {session.sessionNumber}
                    </div>
                    <div>
                      <h4 className="font-bold text-gray-900 text-sm tracking-tight">Session {session.sessionNumber}</h4>
                      <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">{new Date(session.date).toLocaleDateString('en-IN', { day:'2-digit', month:'short', year:'numeric' })}</p>
                    </div>
                  </div>
                  <div className="flex flex-col items-end gap-1">
                    <select
                      ...
                      className="text-[10px] font-bold uppercase tracking-wider px-3 py-1.5 rounded-lg border-transparent focus:ring-2 focus:ring-blue-500/20 bg-white/80 backdrop-blur-sm cursor-pointer outline-none"
                    >
                    ...
                    <div className="text-sm font-bold text-gray-900 mt-1">₹{session.cost.toLocaleString()}</div>
                  </div>
                </div>
                
                <div className="bg-white/40 backdrop-blur-sm rounded-xl p-4 border border-white/60">
                  <p className="text-xs font-semibold text-gray-700 leading-relaxed italic">{session.notes || 'No specific clinical notes added for this session.'}</p>
                  <div className="flex items-center justify-between mt-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest border-t border-gray-100 pt-3">
            ...
            {sessions.length === 0 && (
              <div className="col-span-full text-center py-20 bg-gray-50/50 rounded-3xl border-2 border-dashed border-gray-200">
                ...
                <p className="text-sm font-bold text-gray-400">No treatment sessions scheduled yet.</p>
              </div>
            )}
          </div>

          {showNewSession && (
            <div className="mt-8 bg-blue-50/50 rounded-3xl p-8 border border-blue-100 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
                <div>
                  <label className="form-label text-blue-700">Date</label>
                  <input type="date" value={newSession.date} onChange={(e) => setNewSession({...newSession, date: e.target.value})}
                    className="form-input bg-white border-blue-100" min={new Date().toISOString().split('T')[0]} />
                </div>
                <div>
                  <label className="form-label text-blue-700">Time</label>
                  <input type="time" value={newSession.time} onChange={(e) => setNewSession({...newSession, time: e.target.value})}
                    className="form-input bg-white border-blue-100" />
                </div>
                <div>
                  <label className="form-label text-blue-700">Duration (min)</label>
                  <input type="number" value={newSession.duration} onChange={(e) => setNewSession({...newSession, duration: parseInt(e.target.value)})}
                    className="form-input bg-white border-blue-100" min="15" step="15" />
                </div>
                <div>
                  <label className="form-label text-blue-700">Cost (₹)</label>
                  <input type="number" value={newSession.cost} onChange={(e) => setNewSession({...newSession, cost: parseInt(e.target.value)})}
                    className="form-input bg-white border-blue-100" min="0" />
                </div>
              </div>
              <div className="mb-6">
                <label className="form-label text-blue-700">Session Notes</label>
                <textarea value={newSession.notes} onChange={(e) => setNewSession({...newSession, notes: e.target.value})}
                  rows={2} className="form-input bg-white border-blue-100" placeholder="Clinical objectives for this session..." />
              </div>
