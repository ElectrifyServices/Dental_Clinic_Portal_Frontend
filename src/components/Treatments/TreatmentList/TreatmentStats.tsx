...
export function TreatmentStats({ totals }: TreatmentStatsProps) {
  const stats = [
    { label: 'Total Plans', value: totals.all, icon: <Users className="w-5 h-5" />, color: 'text-gray-900', bg: 'bg-gray-100', border: 'border-gray-200' },
    { label: 'In Progress', value: totals.active, icon: <Clock className="w-5 h-5" />, color: 'text-blue-600', bg: 'bg-blue-100', border: 'border-blue-200' },
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((s) => (
        <div key={s.label} className={`bg-white p-4 rounded-3xl border border-gray-200 shadow-sm transition-all hover:shadow-md group`}>
