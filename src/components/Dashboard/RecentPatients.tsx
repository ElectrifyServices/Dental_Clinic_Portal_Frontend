...
export function RecentPatients() {
  ...
  const STATUS_VARIANT: Record<PatientStatus, 'green' | 'blue' | 'gray'> = {
    active: 'green', new: 'blue', inactive: 'gray',
  };
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between px-5 py-4 border-b border-border">
        <div>
          <h2 className="text-sm font-bold text-foreground">Recent Patients</h2>
          <p className="text-xs text-muted-foreground mt-0.5">Recently registered</p>
        </div>
      ...
      {patients.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 px-4">
          ...
          <h3 className="text-[15px] font-bold text-foreground mb-1 uppercase tracking-tight">Begin your practice</h3>
        ...
        <div className="divide-y divide-border">
          {patients.map((p, i) => (
            <div key={p.id || i} className="flex items-center gap-3 px-5 py-3 hover:bg-muted/50 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <StatusBadge variant={STATUS_VARIANT[(p.status as PatientStatus)] ?? 'gray'}>{p.status || 'new'}</StatusBadge>
                </div>
