...
export const PrescriptionPrintModal: React.FC<PrescriptionPrintModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[60] p-4 backdrop-blur-sm">
      <div className="bg-card rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        <div className="p-6 border-b border-border flex items-center justify-between bg-primary text-white">
          <div className="flex items-center gap-3">
            ...
            <div className="flex bg-primary/50 p-1 rounded-xl border border-primary/30 ml-4">
              <button
                onClick={() => setPrintLanguage("en")}
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${printLanguage === "en" ? "bg-card text-primary shadow-lg" : "text-blue-100 hover:text-white"}`}
              >
                ENGLISH
              </button>
              <button
                onClick={() => setPrintLanguage("gu")}
                className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all ${printLanguage === "gu" ? "bg-card text-primary shadow-lg" : "text-blue-100 hover:text-white"}`}
              >
                ગુજરાતી (GUJ)
              </button>
            </div>
          </div>
          <button onClick={onClose} className="hover:bg-card/10 p-2 rounded-full transition-all">
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6">
              <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide border-b pb-2">Clinical Vitals</h4>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-1">BP (mmHg)</label>
                ...
                <div>
                  <label className="block text-xs font-bold text-muted-foreground uppercase mb-1">Weight (Kg)</label>
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-1">Clinical Complaints / Observation</label>
            ...
            <div className="space-y-6">
              <h4 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide border-b pb-2">Diagnosis & Advice</h4>
              <div>
              ...
              <div>
                <label className="block text-xs font-bold text-muted-foreground uppercase mb-1">Advice / Instructions</label>
              ...
              <button onClick={onPrint} className="w-full bg-primary text-white py-4 rounded-2xl font-bold text-lg hover:bg-primary transition-all flex items-center justify-center gap-3">
