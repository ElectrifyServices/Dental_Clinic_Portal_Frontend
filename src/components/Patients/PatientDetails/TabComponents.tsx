...
const EmptyState = ({ icon: Icon, title, description }: { icon: any, title: string, description: string }) => (
  <div className="flex flex-col items-center justify-center py-20 px-4 bg-muted/50 rounded-3xl border-2 border-dashed border-border">
    ...
    <h3 className="text-lg font-bold text-foreground mb-1 uppercase tracking-tight">{title}</h3>
      ...
      <div className="space-y-3">
        {(patient?.medicalHistory || []).length > 0 ? (
          patient.medicalHistory.map((condition: string, index: number) => (
            <div key={index} className="bg-card rounded-xl p-4 border border-primary/30 shadow-sm">
      ...
      <div className="space-y-3">
        {(patient?.allergies || []).length > 0 ? (
          patient.allergies.map((allergy: string, index: number) => (
            <div key={index} className="bg-card rounded-xl p-4 border border-destructive/20 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {patient.maritalStatus && (
          <div className="bg-card rounded-xl p-4 text-center border border-border">
            ...
            <p className="font-medium text-foreground capitalize">{patient.maritalStatus}</p>
          </div>
        )}
...
export const AppointmentsTab = ({ patientAppointments, getStatusColor }: { patientAppointments: any[], getStatusColor: (s: string) => string }) => (
  <div className="space-y-4">
    ...
    {patientAppointments.map((appointment) => (
      <div key={appointment.id} className="bg-muted rounded-2xl p-6 border border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <Calendar className="w-5 h-5 text-muted-foreground/60 mr-3" />
            <div>
              <p className="font-semibold text-foreground">{appointment.treatmentType || appointment.type}</p>
              <p className="text-sm text-muted-foreground">{new Date(appointment.date).toLocaleDateString()} at {appointment.time}</p>
              <p className="text-sm text-muted-foreground">with {appointment.doctorName || appointment.doctor}</p>
            </div>
          </div>
        ...
        <div>
          ...
          <div className="space-y-3">
            {patientTreatments.filter(t => t.status === 'in-progress').map((treatment) => (
              <div key={treatment.id} className="bg-primary/50 rounded-2xl p-4 border border-primary/20 shadow-sm">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="font-bold text-foreground">{treatment.procedure}</p>
                      <p className="text-xs text-muted-foreground">Tooth: <span className="font-bold">#{treatment.tooth}</span></p>
                      <p className="text-[10px] text-muted-foreground font-mono mt-1">{new Date(treatment.date).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-blue-900">₹{treatment.cost.toLocaleString()}</p>
                    <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-primary text-white uppercase">IN-PROGRESS</span>
                  </div>
                </div>
              </div>
            ))}
              ...
              <div key={treatment.id} className="bg-green-50/30 rounded-2xl p-4 border border-green-100">
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    ...
                    <div>
                      <p className="font-semibold text-muted-foreground">{treatment.procedure}</p>
                      <p className="text-xs text-muted-foreground">Tooth: #{treatment.tooth}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs font-bold text-muted-foreground/60">₹{treatment.cost.toLocaleString()}</p>
...
export const BillingTab = ({ patient, patientInvoices, getStatusColor, handleSendReminder }: { patient: any, patientInvoices: any[], getStatusColor: (s: string) => string, handleSendReminder: () => void }) => (
  <div className="space-y-4">
        ...
        <div className="bg-destructive/10 border border-destructive/20 rounded-xl p-4">
          <div className="flex items-center justify-between gap-6">
            <div>
              ...
              <p className="text-2xl font-bold text-destructive">₹{patient.outstandingBalance.toLocaleString()}</p>
            </div>
            <button onClick={handleSendReminder} className="bg-destructive text-white px-4 py-2 rounded-lg hover:bg-destructive flex items-center text-sm font-medium">
    ...
    {patientInvoices.map((invoice) => (
      <div key={invoice.id} className="bg-muted rounded-2xl p-6 border border-border">
        <div className="flex items-center justify-between">
          <div className="flex items-center">
            <CreditCard className="w-5 h-5 text-muted-foreground/60 mr-3" />
            <div>
              <p className="font-semibold text-foreground">{invoice.id}</p>
              <p className="text-sm text-muted-foreground">Issued: {new Date(invoice.date).toLocaleDateString()}</p>
              <p className="text-sm text-muted-foreground">Due: {new Date(invoice.dueDate).toLocaleDateString()}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="font-bold text-foreground">₹{(invoice.amount || invoice.total || 0).toLocaleString()}</p>
...
export const PrescriptionsTab = ({ patient, handlePrintDocument }: { patient: any, handlePrintDocument: () => void }) => (
  <div className="space-y-4">
    <div className="flex items-center justify-between">
      <h3 className="text-lg font-bold text-foreground">Prescription History</h3>
      {patient.prescriptionHistory?.length > 0 && (
        <button onClick={handlePrintDocument} className="bg-primary text-white px-4 py-2 rounded-xl hover:bg-primary flex items-center text-sm font-medium transition-all duration-200 shadow-lg shadow-blue-200">
          <Printer className="w-4 h-4 mr-2" /> Print Document
        </button>
      )}
    </div>
    {patient.prescriptionHistory?.map((record: any) => (
      <div key={record.id} className="bg-card rounded-2xl p-4 border border-primary/20 shadow-sm hover:shadow-md transition-all duration-200 mb-4">
        <div className="flex items-center justify-between mb-4 pb-4 border-b border-border">
          <div className="flex items-center">
            ...
            <div>
              <h4 className="text-base font-bold text-foreground leading-tight">{record.treatment}</h4>
            ...
            <div key={index} className="bg-muted/50 rounded-xl p-3 border border-border/50">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <h5 className="font-bold text-foreground text-sm">{prescription.medicine}</h5>
                </div>
                <div className="flex items-center gap-2">
                  <div className="flex bg-card rounded-lg border border-border overflow-hidden shadow-sm">
                    <span className="px-2 py-1 text-[10px] bg-muted text-muted-foreground border-r border-border font-bold">DOSAGE</span>
                    <span className="px-2 py-1 text-[11px] text-primary font-bold">{prescription.dosage}</span>
                  </div>
                  <div className="flex bg-card rounded-lg border border-border overflow-hidden shadow-sm">
                    <span className="px-2 py-1 text-[10px] bg-muted text-muted-foreground border-r border-border font-bold">FREQ</span>
                    <span className="px-2 py-1 text-[11px] text-primary font-bold">{prescription.frequency}</span>
                  </div>
                </div>
              </div>
            </div>
...
export const DocumentsTab = ({ patient, loading }: { patient: any, loading: boolean }) => (
  <div>
    <h3 className="text-lg font-bold text-foreground mb-6">Patient Documents & Images</h3>
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {patient.documents?.map((doc: any) => (
        <div key={doc.id} className="bg-card rounded-2xl p-6 border border-border hover:shadow-lg transition-all duration-200">
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-border pb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Information about registered family members under this account</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {familyMembers.map((member) => (
          <div key={member.id} className="bg-card rounded-2xl p-5 border border-border hover:shadow-md transition-all group">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="font-bold text-foreground text-lg group-hover:text-primary transition-colors">{member.name}</p>
                  <div className="flex items-center gap-2">
              ...
              <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${member.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-muted text-muted-foreground'}`}>
            ...
            <div className="grid grid-cols-2 gap-3 mt-4 border-t border-border pt-4">
              <div className="bg-muted rounded-xl p-2.5">
                ...
                <p className="font-bold text-foreground text-sm">{calculateAge(member.dateOfBirth)} Years</p>
              </div>
              <div className="bg-muted rounded-xl p-2.5">
                <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-wider mb-1 flex items-center gap-1">
                  <Calendar className="w-3 h-3" /> Last Visit
                </p>
                <p className="font-bold text-foreground text-sm">{member.lastVisit || 'No visits'}</p>
              </div>
              <div className="bg-muted rounded-xl p-2.5 col-span-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`font-bold text-sm ${member.outstandingBalance > 0 ? 'text-destructive' : 'text-green-600'}`}>
                      ₹{(member.outstandingBalance || 0).toLocaleString()}
                    </p>
                  </div>
                  {member.outstandingBalance > 0 && (
                    <span className="text-[10px] font-black text-red-500 bg-destructive/10 px-2 py-1 rounded uppercase animate-pulse">Payment Due</span>
                  )}
                </div>
              </div>
            </div>
