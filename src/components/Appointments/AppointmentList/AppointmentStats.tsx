...
export const AppointmentStats: React.FC<AppointmentStatsProps> = ({ appointments }) => {
  ...
  const STATS = [
    ...
    { label: 'Upcoming', val: confirmed, icon: Clock, color: 'blue', bg: 'bg-blue-50' },
    { label: 'Completed', val: completed, icon: CheckCircle, color: 'emerald', bg: 'bg-emerald-50' },
    { label: 'Cancelled', val: cancelled, icon: XCircle, color: 'red', bg: 'bg-red-50' }
  ];

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
      {STATS.map((s, i) => (
        <div key={i} className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-xl shadow-gray-200/20 group hover:border-primary/20 transition-all duration-300">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <h4 className="text-2xl font-black text-gray-900 tracking-tight">{s.val}</h4>
            </div>
          </div>
        </div>
      ))}
