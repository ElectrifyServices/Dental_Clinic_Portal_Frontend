...
// Sub-components
import { ConsultationHeader } from "./PatientConsultation/ConsultationHeader";
...
export function PatientConsultation({
  ...
  const getAvailableTimeSlots = () => {
    ...
    if (!daySchedule || !daySchedule.isWorking || !doctorAvailability[followUpDoctorId])
      return [];
    ...
    return allSlots.filter((slot) => {
      ...
      if (slotHour < startHour || slotHour > endHour || (slotHour === endHour && slotMinute > endMinute)) return false;
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setConsultationData((prev) => ({ ...prev, images: [...prev.images, ...imageUrls] }));
  };

  const handleXrayUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    const imageUrls = files.map((file) => URL.createObjectURL(file));
    setConsultationData((prev) => ({ ...prev, xrayFiles: [...prev.xrayFiles, ...imageUrls] }));
  };

  const removeImage = (index: number) => {
    setConsultationData(prev => ({ ...prev, images: prev.images.filter((_, i) => i !== index) }));
  };

  const removeXrayFile = (index: number) => {
    setConsultationData(prev => ({ ...prev, xrayFiles: prev.xrayFiles.filter((_, i) => i !== index) }));
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div id="consultation-form" className="bg-card rounded-2xl max-w-5xl w-full max-h-screen overflow-y-auto shadow-2xl relative custom-scrollbar">
        {isCompleted ? (
          ...
          <>
            <ConsultationHeader patient={patient as any} onClose={onClose} />
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-8 pb-20">
              ...
              <TreatmentPlanning 
                ...
                treatmentCost={consultationData.treatmentCost === 0 ? "" : consultationData.treatmentCost}
              ...
              <div className="sticky bottom-0 bg-card/90 backdrop-blur-md flex justify-end space-x-4 p-6 border-t border-border rounded-b-2xl z-30 -mx-6 -mb-6 shadow-[0_-10px_20px_-15px_rgba(0,0,0,0.1)]">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="px-8 py-3.5 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-2xl hover:from-green-700 hover:to-emerald-700 font-bold flex items-center shadow-xl hover:shadow-green-200 transition-all disabled:opacity-50 active:scale-95"
                >
                  {loading ? (
                    <>
