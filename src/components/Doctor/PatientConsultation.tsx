...
export function PatientConsultation({
  ...
  useEffect(() => {
    ...
    setConsultationData(prev => {
      ...
      // Filter out plans for teeth no longer selected
      const filteredPlans = existingPlans.filter((plan: any) => 
        selectedToothNums.includes(parseInt(plan.tooth))
      );
      
      // Add new plans for newly selected teeth
      const newTeeth = selectedToothNums.filter(num => 
        !filteredPlans.find((p: any) => parseInt(p.tooth) === num)
      );
      ...
      const updatedPlans = [...filteredPlans, ...newPlans].sort((a, b) => parseInt(a.tooth) - parseInt(b.tooth));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Note: handleCompleteConsultation now handles processing treatmentPlans
              ...
              <div className="bg-purple-50 rounded-2xl p-6 border border-purple-200">
                ...
                {consultationData.requiresTreatment && (
                  <div className="mt-4 overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-purple-200">
                          <th className="py-2 px-3 text-xs font-bold text-purple-900 uppercase">Tooth</th>
                          <th className="py-2 px-3 text-xs font-bold text-purple-900 uppercase">Procedure</th>
                          <th className="py-2 px-3 text-xs font-bold text-purple-900 uppercase">Sessions</th>
                          <th className="py-2 px-3 text-xs font-bold text-purple-900 uppercase">Est. Cost (₹)</th>
                          <th className="py-2 px-3 text-xs font-bold text-purple-900 uppercase text-center">Start Today</th>
                        </tr>
                      </thead>
                      <tbody>
                        {consultationData.treatmentPlans.map((plan: any, index: number) => (
                          <tr key={plan.id} className="border-b border-purple-100 hover:bg-purple-100/50 transition-colors">
                            <td className="py-3 px-3">
                              <span className="font-bold text-purple-900">#{plan.tooth}</span>
                            </td>
                            <td className="py-3 px-3">
                              <select
                                ...
                                onChange={(e) => {
                                  ...
                                  updatedPlans[index].procedure = e.target.value;
                                  setConsultationData({ ...consultationData, treatmentPlans: updatedPlans });
                                }}
                                className="w-full px-2 py-1.5 text-sm border border-purple-200 rounded-lg focus:ring-2 focus:ring-purple-500 bg-white"
                              >
                            ...
                            <td className="py-3 px-3">
                              <input
                                ...
                                onChange={(e) => {
                                  const updatedPlans = [...consultationData.treatmentPlans];
                                  updatedPlans[index].sessions = parseInt(e.target.value) || 1;
                                  setConsultationData({ ...consultationData, treatmentPlans: updatedPlans });
                                }}
                                className="w-16 px-2 py-1.5 text-sm border border-purple-200 rounded-lg focus:ring-2 focus:ring-purple-500 bg-white"
                              />
                            </td>
                            <td className="py-3 px-3">
                              <input
                                ...
                                onChange={(e) => {
                                  const updatedPlans = [...consultationData.treatmentPlans];
                                  updatedPlans[index].cost = parseInt(e.target.value) || 0;
                                  setConsultationData({ ...consultationData, treatmentPlans: updatedPlans });
                                }}
                                className="w-24 px-2 py-1.5 text-sm border border-purple-200 rounded-lg focus:ring-2 focus:ring-purple-500 bg-white"
                                placeholder="Cost"
                              />
                            </td>
                            <td className="py-3 px-3 text-center">
                              <input
                                ...
                                onChange={(e) => {
                                  const updatedPlans = [...consultationData.treatmentPlans];
                                  updatedPlans[index].isActive = e.target.checked;
                                  setConsultationData({ ...consultationData, treatmentPlans: updatedPlans });
                                }}
                    ...
                    {consultationData.treatmentPlans.length === 0 && (
